<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Компании</h2>
    </x-slot>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="flex flex-wrap">
                            <div class="w-full md:w-2/3 text-center md:text-left">{{ $company->name }}</div>
                            <div class="w-full md:w-1/3 flex justify-center md:justify-end">
                                <div class="px-2 text-green-600">
                                    <a href="{{ route('admin.company.edit', $company->id) }}">Править</a>
                                </div>
                                <div class="px-2 text-red-600">
                                    <form action="{{ route('admin.company.destroy', $company->id) }}" method="POST">
                                        @csrf()
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button onclick="return confirm('Удалить компанию?')">
                                            Удалить
                                        </button>
                                    </form>
                                </div>
